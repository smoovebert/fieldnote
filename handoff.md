...
- Eight-tier type hierarchy (2026-05-03, seven sequential commits, no functional change): translates the design handoff at `design_handoff_type_hierarchy/README.md` into the existing token vocabulary in `src/styles/tokens.css`. The new scale: T1 Newsreader 32/1.15/500 -0.018em (page-defining titles), T2 Inter Tight 22/1.2/500 -0.012em (mode-only page titles), T3 Inter Tight 16/1.35/500 (in-card section heads), T4 Inter Tight 14/1.4/500 (subhead, code names), T5 Inter Tight 13.5/1.5/400 (body baseline — color also shifted from --ink to --ink-2 so T1-T4 elements pop), T6 Inter Tight 12.5/1.45/400 (body-small, table cells), T7 Inter Tight 10.5/1.2/600 0.10em uppercase (eyebrow / panel-head / table column header), T8 JetBrains Mono 11/1.3/400 (counts, dates, refs). Old short-hand names (`--t-display`, `--t-title`, `--t-ui-lg`, `--t-ui`, `--t-ui-sm`, `--t-label`, `--t-meta`) remain as back-compat aliases pointing at the closest tier so 46+ existing call sites pick up the recalibration without a mass rename. Applied per-page per the spec: title-input → T1 (subject pages: Overview/Organize/Code/Refine), static-detail-title → T2 (mode pages: Classify/Analyze/Report), .detail-toolbar .eyebrow → T7. Right-rail panel heads → T7; properties dl gets dt = T6 muted + dd = T4 dark with rule-soft per-row underlines so the value pops over the label. Editorial Report layout: 68ch container, 44px Newsreader 500 H1 (the largest type in the entire app), T7 H2 section heads, T8 mono meta line, codebook entries become serif name + italic Newsreader 14/1.55 description with 22px gap. Top nav: F mark hidden, two-line wordmark (eyebrow + brand line), tabs adopt 32px height + 6px radius + rgba-white-0.08 active background. Left rail .pane-title → T7 in --shell-ink-3. Content surfaces: generic h2 → T3, transcript reader = Newsreader 16.5/1.75 + line numbers T8 mono in --ink-4, .analyze-table + .crosstab-table = T7 column heads + T6 cells + T4 row-title cells. Newsreader 600 weight added to the index.html font URL for editorial codebook serif names. Spec lives in `design_handoff_type_hierarchy/README.md`; the per-tier definitions live in `src/styles/tokens.css` as `--t-t1` through `--t-t8`.
