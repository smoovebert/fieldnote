...
- The terms doc lives in two places that must stay byte-identical: `docs/terms-of-service.md` (repo source of truth, linked from README) and `public/terms-of-service.md` (served by Vite from `/terms-of-service.md` for the in-app link). No automated sync today — diverging silently is the failure mode; future cleanup options are a symlink or a CI diff check.
...
- Alpha Terms-of-Service gate (2026-05-03): sign-up requires explicit acceptance of the Fieldnote Alpha Terms of Service via a checkbox; the Create-account button stays disabled until ticked. Sign-in shows a softer "by signing in, you continue under the terms" reminder. Both link to the deployed `/terms-of-service.md` asset (also present at `docs/terms-of-service.md` for the repo source-of-truth). New sign-ups persist `termsAcceptedAt` (ISO timestamp) and `termsVersion` (`'2026-05-02-alpha'`) into Supabase auth metadata so each tester's accepted version is auditable. The two TOS files must stay byte-identical — manual sync today; symlink or CI diff check is the future cleanup.
