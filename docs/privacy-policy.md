...
- **Delete your account**: open the profile menu (User icon, top right of the app) → "Delete account…" → re-type your email + tick the confirmation checkbox → "Delete my account". The deletion is immediate, server-side, and permanent: your auth record is removed and every related row (projects, sources, codes, excerpts, memos, cases, attributes, saved queries, snapshots, AI usage logs, AI call audit rows, BYOK encrypted keys) cascades automatically. We cannot recover any of it afterwards. Email **[studio.ops@behemothagency.com](mailto:studio.ops@behemothagency.com)** if the in-app flow ever fails.
- **Withdraw AI consent**: open the profile menu → "AI assist settings" → switch to BYOK. Once you switch, no further free-tier prompts will be sent to Google. Existing logged calls remain in the audit table; you can request their removal with the same email above.
